...
try:
    from dspy.clients.qwen import inspect_qwen_runtime, probe_qwen_runtime
except ModuleNotFoundError as exc:
    missing = exc.name or "a required package"
    raise SystemExit(
        f"Missing Python dependency: {missing}. "
        "Run `uv sync --extra mcp --extra dev` once, or invoke this script with "
        "`uv run python scripts/dspy_qwen_doctor.py`."
    ) from exc


def main() -> int:
    parser = argparse.ArgumentParser(description="Inspect local Qwen CLI runtime readiness.")
    ...
    if args.live:
        payload["probe"] = probe_qwen_runtime(
    ...
    if args.json:
        ...
        print(f"Qwen CLI: {runtime.cli_path or 'missing'}")
        print(f"Qwen home: {runtime.qwen_home}")
